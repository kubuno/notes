...
#[derive(Parser, Debug)]
#[command(name = "kubuno-notes", version, about = "Module notes Kubuno")]
struct Cli {
    #[arg(short, long, env = "KN_CONFIG_FILE")]
...
async fn main() -> Result<()> {
    ...
    match settings.logging.format {
        kubuno_notes::config::LogFormat::Json   => subscriber.json().init(),
        kubuno_notes::config::LogFormat::Pretty => subscriber.init(),
    }

    tracing::info!("Kubuno Notes v{} démarrage…", env!("CARGO_PKG_VERSION"));

    // Sécurité : interdire toute exécution de processus sur l’hôte (voir kubuno-seccomp).
    kubuno_seccomp::lock_down_process_execution("notes");

    // Pool PostgreSQL
    let opts = settings.database.connect_options()?
        .options([("search_path", "notes,public")]);
    ...
    if settings.database.run_migrations {
        sqlx::query("CREATE SCHEMA IF NOT EXISTS notes")
            .execute(&pool)
            .await
            .context("Création du schéma notes")?;
    ...
    let http = Client::new();
    let files_client = Arc::new(kubuno_notes::files_client::FilesClient::new(
    ...
    // Heartbeat toutes les 30s
    {
        ...
        tokio::spawn(async move {
            loop {
                ...
                let url    = format!("{}/internal/modules/notes/heartbeat", settings2.core.url);
    ...
    // Worker de rappels
    {
        ...
        let core    = settings.core.url.clone();
        let secret  = settings.core.internal_secret.clone();
        let interval = settings.notes.reminder_check_interval;
        tokio::spawn(async move {
            reminder_worker::start(db2, http3, core, secret, interval).await;
        });
    }
    ...
    tracing::info!("Kubuno Notes démarré sur http://{addr}");
...
async fn register_with_core(http: &Client, settings: &Settings) {
    ...
    let display_name  = manifest.as_ref().map(|m| m.module.display_name.as_str()).unwrap_or("Notes").to_string();
    ...
    for attempt in 1u32.. {
            ...
            .await
        {
            Ok(resp) if resp.status().is_success() => {
                tracing::info!("Module notes enregistré auprès du core");
                return;
            }
